class ImageInput < SimpleForm::Inputs::Base
  DEFAULT_IMAGE = 'card-default.png'.freeze
  CONTROLLER = 'input-image'.freeze
  ...
  def wrapper_attributes
    {
      class: 'relative w-full h-64 border-2 border-dashed border-gray-300 rounded-lg bg-white',
  ...
  def upload_content
    template.content_tag :div, class: 'text-center px-4' do
      template.icon('fa-solid', 'cloud-arrow-up', class: 'fa-3x mb-6 text-gray-400') +
        template.content_tag(:p, 'Clique para enviar', class: 'mb-3 text-sm text-gray-500 font-semibold')
    end
  end
  ...
  def upload_label_attrs
    classes = 'absolute inset-0 flex flex-col items-center justify-center rounded-lg cursor-pointer hover:bg-gray-50'
    {
      class: "#{classes} #{valid_attachment? ? 'hidden' : ''}",
